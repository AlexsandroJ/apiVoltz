...
#define TESTMODE                                                               \
  false // Se true, gera dados aleatórios para teste sem hardware CAN
#define DEBUGMODE false
#define BufferSize                                                             \
  250 // Buffer aumentado para evitar perda em latências de rede
...
void canSourceTask(void *pvParameters) {
  for (;;) {
    ...
    if (TESTMODE) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        frame.timestamp =
            (int64_t)tv_now.tv_sec * 1000LL + (tv_now.tv_usec / 1000LL);
      ...
      if (hasData) {
        ...
        if (xQueueSend(canRawQueue, &frame, 0) != pdTRUE) {
          if (DEBUGMODE)
            Serial.println("Fila de processamento cheia!");
        }
      }
  ...
  void setup() {
    ...
    xTaskCreatePinnedToCore(canSourceTask, "CAN_Source", 4096, NULL, 3, NULL,
                            0);

    // Task de Wi-Fi/MQTT no Core 1 (Prioridade 1 - Menor)
    xTaskCreatePinnedToCore(mqttPublisherTask, "MQTT_Pub", 8192, NULL, 1, NULL,
                            1);
  }
